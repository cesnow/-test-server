...
func (q *sessionOutgoingQueue) AddNotifyMessage(notifyId int64, confirm bool, msg *transport.TMsgRawData) *outboxMsg {
	oMsg := new(outboxMsg)
	oMsg.msgId = notifyId
	oMsg.sent = 0
	if confirm {
...
func (q *sessionOutgoingQueue) AddPushUpdates(pushMsgId int64, result *transport.TMsgRawData) *outboxMsg {
