...
func MustNewModel(db *DB, collection string, opts ...Option) *Model {
	model, err := NewModel(db, collection, opts...)
	logx.Must(err)
	return model
}

// NewModel returns a Model.
func NewModel(db *DB, collection string, opts ...Option) (*Model, error) {
	name := strings.Join([]string{db.config.URI, collection}, "/")
	brk := breaker.GetBreaker(db.config.URI)
	coll := newCollection(db.client.Database(db.config.Database).Collection(collection), brk)
