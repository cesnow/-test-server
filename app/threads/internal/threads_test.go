...
func TestClient_GetUserID(t *testing.T) {
	...
	client, err := NewClient(ctx, WithClient(&http.Client{Transport: &fake{}}), WithToken(token))
	if err != nil {
		t.Fatal(err)
	}
	for _, tt := range tests {
...
func TestClient_Page(t *testing.T) {
	...
	// 1) 建 client（不帶 Token 也可抓公開端點）
	client, err := NewClient(ctx)
	if err != nil {
		log.Fatal(err)
	}

	// 2) 用 username 先換 user ID
	uid, err := client.GetUserID(ctx, "netflixtw")
	if err != nil {
		log.Fatalf("GetUserID: %v", err)
	}
	fmt.Println("userID:", uid)
	//
	//got, _ := client.GetUserThreads(ctx, uid)
	//var pretty bytes.Buffer
	//_ = json.Indent(&pretty, got, "", "\t")
	//t.Log(pretty.String())
	ok := client.DocIDSupportsAfter(ctx, getUserThreadsDocID, uid)
	if !ok {
		fmt.Println("errrr")
		// 這支 doc_id 不吃分頁，必須換
	}
	//// 單頁
	//raw, err := client.GetUserThreadsPage(ctx, uid, 1, "")
	//if err != nil {
	//	panic(err)
	//}
	//fmt.Println(string(raw))
	//// 全抓（建議 delay 500~1000ms 更穩）
	//pages, err := client.GetAllUserThreads(ctx, uid, 20, 3, 800*time.Millisecond)
	//if err != nil {
	//	panic(err)
	//}
